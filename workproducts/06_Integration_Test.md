...
실제 ECU 보드, 실제 CAN Bus, PWM 핀, LED 및 모터는 본 SWE.5 시험 범위에 포함하지 않는다. 해당 외부 환경은 Stub/Mock으로 대체하며 실제 하드웨어를 포함하는 검증은 SWE.6 및 시스템시험에서 수행한다.
...
| 제외 대상 | 후속 검증 단계 |
|---|---|
| 실제 입력 ECU와 출력 ECU 사이의 CAN 통신 | SWE.6 / 시스템시험 |
| 실제 10 ms Task 주기 및 OS Scheduling | SWE.6 / 시스템시험 |
| 실제 PWM 파형, Digital Pin, LED 및 모터 동작 | 시스템시험 |
| 전원, 배선 및 CAN Transceiver 고장 | 시스템시험 |
| 실제 CANoe Network Fault Injection | SWE.6 / 시스템시험 |
