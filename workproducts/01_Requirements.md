...
| 시스템 요구사항 | 기능·설계 문서 전개 대상 | 구현·검증 전개 대상 |
|---|---|---|
| Req_001~Req_004 | 조향 입력, CAN 전달 및 CanMonitor 기능 | 통신 명세, 입력 진단 단위·통합·시스템 시험 |
| Req_005 | WdgM 기반 실행 감시 기능 | WdgM 설정, 실행 이상 Fault Injection 시험 |
...
CAN ID, DLC, 10 ms 송신 주기, 조향각 범위, Alive Counter 반복 횟수, 정상 복귀 횟수, WdgM 상태값, 방향 판정 임계값 및 PWM 계산식은 본 시스템 요구사항의 구현 방법에 해당하므로 `03~04` 설계·구현 문서에서 관리한다.
