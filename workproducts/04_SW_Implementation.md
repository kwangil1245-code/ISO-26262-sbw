...
본 문서는 `0301_SW_Architecture_Design.md`에서 정의한 SWC, Interface 및 Runnable을 구현 가능한 소프트웨어 단위로 상세화한다. 각 단위의 입출력, 내부 상태, 상세 처리 로직, RTE·WdgM·IoHwAb 연동 및 소스 구성을 정의하고 `SWR-*`까지의 추적성을 유지한다.
